...
// =====================================================================
// Natok Title pre-renderer (Emoji Original Color Fixed)
// =====================================================================
    ...
    for seq, is_em in get_chunks(ln):
        if is_em and emoji_font:
            ...
            try:
                ...
                # যদি Pillow এর খুব পুরনো ভার্সন হয়
...
function renderTitlePng(opts) {
  ...
  if (r.status !== 0) {
    throw new Error(`Natok Title render failed: ${r.stderr || r.stdout || 'python3 exited ' + r.status}`);
  }
  if (!fs.existsSync(outPath)) {
    throw new Error('Natok Title render: output PNG not created');
  }
  return outPath;
}
